...
export const invertString = (inString: string): string[] => {
  let swapString: string[] = [];
...
export const StringComponent: React.FC = () => {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsLoader(true);
    swapString = invertString(inputString);//расчет значений
    setCurrentValue(0);
    reverseStr(inputString, swapString);  //отображение
  }

  //функция прокрутки во времени массива строк
  const reverseStr = async (inString: string, swapString: string[]) => {
